...
from structured_diag.interpretation.arbitrator_prompts import (
    JSON_SCHEMA_BLOCK,
    PROMPT_VERSION,
...
from structured_diag.interpretation.llm_interpreter import (
    _cache_load,
    _cache_store,
    _parse_json_block,
)
    ...
    *,
    ...
    return (
        ArbitrationDecision(
            ...
            chosen_source=chosen_source,  # type: ignore[arg-type]
    ...
    *,
    ...
    if decision.chosen_label in (flat_label, cascade_label):
        ...
        if decision.chosen_source != expected_source:
            decision = replace(
                decision,
                chosen_source=expected_source,  # type: ignore[arg-type]
            )
        return decision
    ...
    return replace(
        ...
        chosen_source=snapped_source,  # type: ignore[arg-type]
    ...
    *,
    ...
    for backend in chain:
        if backend == "template":
            ...
            if cached is not None:
                ...
                return ArbitrationDecision(
                    ...
                    chosen_source=str(cached.get("chosen_source") or "flat"),  # type: ignore[arg-type]
        ...
        if cached is not None:
            ...
            return ArbitrationDecision(
                chosen_label=cached_label,
                chosen_source=str(cached["chosen_source"]),  # type: ignore[arg-type]
                confidence=cached_conf,
